...
def db_health():
    try:
        ...
        with get_connection() as conn, conn.cursor() as cur:
            ...
            row = cur.fetchone()
            users_exists, accounts_exists, transactions_exists, receipts_exists, categories_exists, email_codes_exists, refresh_tokens_exists = row
...
def check_auth():
    try:  
        ...
        with get_connection() as conn, conn.cursor() as cur:
            cur.execute(
                    """
                    SELECT email, code_hash, expires_at, attempts_left, used, verified_at FROM main.email_codes
                    """
                )
            row = cur.fetchone()
...
def check_auth():
    try:  
        ...
        with get_connection() as conn, conn.cursor() as cur:
            cur.execute(
                    """
                    SELECT id, user_id, token_hash, jti, created_at, expires_at, revoked FROM main.refresh_tokens
                    """
                )
            row = cur.fetchone()
