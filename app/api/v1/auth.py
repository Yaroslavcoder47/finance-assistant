...
from app.schemas.auth import (
    EmailIn, RequestCodeOut,
    CodeVerifyIn, CodeVerifyOut,
    SetPasswordIn, LoginIn, 
    TokensOut, RefreshIn, 
    LogoutIn
)
...
def request_code(body: EmailIn):
    ...
    with get_connection() as conn, conn.cursor() as cur:
        cur.execute("""
            INSERT INTO main.email_codes (email, code_hash, expires_at, attempts_left, used, verified_at)
...
def verify_code(body: CodeVerifyIn):
    ...
    with get_connection() as conn, conn.cursor() as cur:
        ...
        if code_h != db_hash:
            ...
            cur.execute("UPDATE main.email_codes SET attempts_left = attempts_left - 1 WHERE email = %s", (email,))
            raise HTTPException(400, "Invalid code")

        cur.execute("UPDATE main.email_codes SET used = TRUE, verified_at = %s WHERE email = %s", (now, email))
...
def set_password(body: SetPasswordIn):
    ...
    with get_connection() as conn, conn.cursor() as cur:
        cur.execute("SELECT verified_at FROM main.email_codes WHERE email = %s AND used = TRUE", (email,))
        row = cur.fetchone()
...
@router.post("/refresh", response_model=TokensOut, summary="Update access/refresh tokens with refresh")
